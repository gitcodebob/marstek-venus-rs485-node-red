...
- When asked to commit, or "commit and push", always:
    ...
    1. Use the approved commit message to update the `RELEASE_NOTES.md` if applicable.
        ...
        1. Update the release notes with the specific files changed `home assistant\` and `node-red\` folders. Omit documentation files (e.g.`README.md`, `RELEASE_NOTES.md`, `docs\`).
        1. Also update the `Home Battery Control (v <version>)` in `home assistant\dashboard.yaml` to match the changes and current SemVer.
        1. Also update the `"label": <name> v <version>` for each json file in `node-red\` to match the changes and current SemVer.
